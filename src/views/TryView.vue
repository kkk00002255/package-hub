...
const messages = ref([
  { role: 'system', text: 'Demo workspace — your messages here are not persisted and not sent to any model yet. Wire up your API key in `src/api/` to go live.' }
])
...
function send() {
  ...
  sending.value = true
  setTimeout(() => {
    messages.value.push({ role: 'assistant', text: `(${selected.value.name} would respond here. Plug in your upstream API key — see README.)` })
    sending.value = false
  }, 900)
}
</script>
      ...
      <div class="grid gap-6 lg:grid-cols-[280px_1fr]">
        ...
        <aside class="lg:sticky lg:top-24 lg:self-start">
          <div class="grad-border p-4 max-h-[78vh] overflow-y-auto">
            <div class="text-[11px] uppercase tracking-wider text-ink-500 px-1 pb-2">Models</div>
            <div v-for="g in grouped" :key="g.id" class="mt-3">
              ...
              >
                ...
                <span v-if="m.tier === 'flagship'" class="text-[9px] uppercase text-cyan-300 shrink-0">★</span>
              </button>
            </div>
          </div>
        </aside>
        ...
        <div class="grad-border flex flex-col min-h-[78vh]">
          <div class="flex items-center justify-between px-5 py-3 border-b border-white/5">
            <div>
              <div class="text-xs text-ink-500">Workspace</div>
              <div class="text-sm font-semibold text-white">{{ selected.name }} <span class="text-ink-500 font-normal">· {{ selected.vendor }}</span></div>
            </div>
            <span class="chip"><span class="h-1.5 w-1.5 rounded-full bg-emerald-400"></span> Ready</span>
          </div>
          ...
          <div class="flex-1 px-5 py-6 space-y-4 overflow-y-auto">
            ...
            >
              ...
              >{{ m.text }}</div>
              <div v-else class="mx-auto max-w-md text-center text-[11px] text-ink-500 italic">{{ m.text }}</div>
            </div>
            <div v-if="sending" class="flex justify-start">
              <div class="bg-white/5 border border-white/10 rounded-2xl px-4 py-2.5 inline-flex items-center gap-2 text-sm text-ink-300">
                <Loader2 class="h-3.5 w-3.5 animate-spin" /> {{ selected.name }} is thinking…
              </div>
            </div>
          </div>
          ...
          <form @submit.prevent="send" class="border-t border-white/5 p-4">
            ...
            <p class="mt-2 px-1 text-[11px] text-ink-500">
              <Sparkles class="inline h-3 w-3 -mt-0.5" /> Demo only. Connect your upstream key in <code class="text-ink-400">src/api/</code> to send real requests.
            </p>
          </form>
        </div>
      </div>
